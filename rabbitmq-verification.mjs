...
try {
  ...
  created.companyId = await createCompany(
    { name: "MQ Verification", location: "Jakarta", description: "Verification" },
    created.ownerId
  );
  ...
  const application = await createApplication(created.applicantId, created.jobId);
  ...
  console.log(JSON.stringify({ application, notification, duplicateStatus, missingJobStatus }));
} finally {
  if (created.categoryId) await pool.query("DELETE FROM categories WHERE id = $1", [created.categoryId]);
  if (created.companyId) await pool.query("DELETE FROM companies WHERE id = $1", [created.companyId]);
  if (created.ownerId || created.applicantId) {
    await pool.query("DELETE FROM users WHERE id = ANY($1)", [[created.ownerId, created.applicantId].filter(Boolean)]);
  }
  await pool.end();
}
